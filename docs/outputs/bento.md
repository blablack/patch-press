...
**18 is a hard cap here too, not just an observation about the factory bank — confirmed on hardware.** The browser row doesn't scroll or wrap a name that's too long to fit; it just clips it. Without an enforced limit that's actively harmful rather than cosmetic: the prefix (below) is identical for every preset out of one source folder, so once a joined name runs past the row width, every preset in that folder clips to the same visible text and they become indistinguishable on the device — several patches all showing, say, `SFM VS Keys Pads -`. `_MAX_NAME = 18` in `bento.py` is that cap, and the budget goes to the preset's own name first: the prefix shrinks, never the name, since the prefix carries no distinguishing information within a folder anyway. How it shrinks is the subject of "Fitting the row" below.
...
That's still 46 characters, well past the 18-character cap — the labels only get it into the neighborhood, and "Fitting the row" below does the last mile, landing on `VS - Polaris Space Delay Soft D2`. Four rules produce the labels, in `_prefix_labels`/`_abbreviate`:
...
The joined `SFM VS Keys Pads - Polaris Space Delay Soft D2` is cut down to the row a label at a time, from the outside in. `_folder_candidates` lists every name the preset could go by, shortest first: the name behind the longest run of its *innermost* labels that still fits 18 characters (always at least one label — the pack the preset is actually from), then one more label outward per step, up to the full prefix. Labels go outermost-first because the collection tag is what a thousand patches share and the innermost label is the one that says which pack. The name is never clipped and neither is a label: two earlier versions did — `name[:18]` put "TUC The Great Zorp I" and "II" in one folder and mangled "HS Chord Bass - minor" to "HS Chord Bass - mi"; `prefix[:room]` produced `SFM Junos B - Acid` and, once the name alone filled the row, no prefix at all. So the row width is a soft target once the name and its pack fill it, not a byte budget worth breaking uniqueness or readability for. Across the 3000-preset corpus about half the names fit in 18; the rest are name-bound.

**A folder that only says what the tag says stays out of the prefix.** `1 BASS`, `01. Keys & Pads`, `02. Kits`, `Orchestral Brass`, `Dream Synth` — every word of these is in `bento_index`'s tag vocabulary (`said_by_tags`), so the browser's tag filter already carries them; `Bass - HS Cable` next to a `Bass` tag spends a third of the row saying it twice. The ladder therefore runs the *untagged* labels first (`Junos - E Piano`, `SFM Junos - E Piano`) and only then the full run with the tag folders back in (`Keys - E Piano`, `Junos Keys - E Piano`, …), because for a percussion hit the tag folder is sometimes the only thing that tells it from its neighbour (`808/Individual Hits/Claves/Clean` vs `…/Cowbell/Clean` → `IH Claves - Clean` / `IH Cowbell - Clean`). A label with any word the vocabulary doesn't know (`Individual Hits`, `Vinyl Synths`, `808 Bass`) is kept whole — dropping only its tag words would fold `Vinyl Synths` and `Vinyl Drums` into one `Vinyl`. The name still loses its echo of a dropped folder, as of any other (`Orchestral Brass Full Ensemble Marcato` → `OT - Full Ensemble Marcato`). Over the corpus this leaves 48 of 2994 names with a tag word in the prefix, all forced by a lookalike.

**Which label tells two presets apart is not knowable one preset at a time.** `Keys - E Piano` is Junos *and* Mirage; `IH - 01 Bass Drum` is six drum packs; `01 Clean Kit 01` is `808 From Mars/Kits` *and* `Essential WAV/Drums/Kits/808`. The version that dropped the prefix to nothing silently wrote the second of each pair over the first — 123 folders lost out of 2690 on the first build with it, exactly the bug the cap exists to prevent, moved one step left. So the first candidate is only a starting point. `assign_output_folders` names a whole set together: everything starts at its shortest candidate, and each round every member of a clash — all of them, so a clashing pair both lengthen rather than one side keeping the short name — steps up one candidate, until nothing clashes or nobody can step further. The result depends only on the set, so the same corpus always gets the same names; adding a pack can lengthen an existing name that just gained a lookalike, which is the price of not fixing names for life at first sight. The chosen name is written to the config's `output.folder`, which `expected_outputs` and `export` honour verbatim — the same field is the manual override for a clash nothing automatic resolves (two presets identical down to the full source path).

Who calls it: `patch-press batch` names its own set before deciding what to skip (configs that already carry a `folder` keep it — a driver that runs `batch` on one slice of a bigger corpus has named the corpus itself), then refuses to build at all if two configs still claim one folder, listing both. A driver with more than one batch's worth of presets in one flat `UserPatches/` has to make the assignment itself, across everything, and persist it; prepare-sd-cards does (`lib/build_presets.py`'s `assign_output_folders`), and renames an already-built patch folder in place when its name changes rather than rebuilding it — `patch.xml` never mentions its own folder, and `bento_index.move_entries` carries the tags over.
