...
def _patch_cables(parent) -> None:
    ...
    etree.SubElement(cables, "patchCable", source="velocity", destination="volume",
                     amount="0x3FFFFFE8")
    etree.SubElement(cables, "patchCable", source="aftertouch", destination="volume",
                     amount="0x2A3D7094")
    etree.SubElement(cables, "patchCable", source="y", destination="lpfFrequency",
                     amount="0x19999990")
...
class DelugeExporter:
    def export(self, sset: SampleSet, config: OutputConfig) -> Path:
        wav_dir = config.path / "SAMPLES" / config.name
        xml_path = config.path / f"{config.name}.xml"
    ...
    def _export_multisample(
        ...
        for s in sset.samples:
            if s.note not in by_note or abs(s.velocity - 100) < abs(
                by_note[s.note].velocity - 100
            ):
                by_note[s.note] = s
        ...
        dp = etree.SubElement(
            ...
            lpfFrequency="0x7FFFFFFF", lpfResonance="0x80000000",
            hpfFrequency="0x80000000", hpfResonance="0x80000000",
            lfo1Rate="0x1999997E", lfo2Rate="0x00000000",
            modFXRate="0x00000000", modFXDepth="0x00000000",
            delayRate="0x00000000", delayFeedback="0x80000000",
            reverbAmount="0x80000000", arpeggiatorRate="0x00000000",
            stutterRate="0x00000000", sampleRateReduction="0x80000000",
            bitCrush="0x80000000", modFXOffset="0x00000000", modFXFeedback="0x00000000",
        )
        etree.SubElement(dp, "envelope1", attack="0x80000000", decay="0xE6666654",
                         sustain="0x7FFFFFFF", release="0x02000000")
        etree.SubElement(dp, "envelope2", attack="0xE6666654", decay="0xE6666654",
    ...
    def _export_kit(
        ...
        for note in sorted(by_note):
            ...
            sound = etree.SubElement(
                sources, "sound",
                name=inst_name,
                polyphonic="auto", voicePriority="1", sideChainSend="2147483647",
                mode="subtractive", lpfMode="24dB", modFXType="none",
            )
            for i, s in enumerate(note_samples):
                ...
                osc = etree.SubElement(
                    ...
                    type="sample",
                    loopMode="1" if s.loop_points else "0",
                    reversed="0", timeStretchEnable="0", timeStretchAmount="0",
                    fileName=_deluge_path(wav),
                )
                _zone_el(osc, wav, s)
            ...
            sdp = etree.SubElement(
                sound, "defaultParams",
                arpeggiatorGate="0x00000000", portamento="0x80000000",
                compressorShape="0xDC28F5B2", oscAVolume="0x00000000",
                oscAPulseWidth="0x00000000", oscBVolume="0x00000000",
                oscBPulseWidth="0x00000000", noiseVolume="0x80000000",
                volume="0x4CCCCCA8", pan="0x00000000",
                lpfFrequency="0x7FFFFFFF", lpfResonance="0x80000000",
                hpfFrequency="0x80000000", hpfResonance="0x80000000",
            ...
            etree.SubElement(sdp, "equalizer", bass="0x00000000", treble="0x00000000",
                             bassFrequency="0x00000000", trebleFrequency="0x00000000")
            etree.SubElement(sound, "arpeggiator", mode="off", numOctaves="2", syncLevel="7")
            ...
            etree.SubElement(cables, "patchCable", source="y", destination="lpfFrequency",
                             amount="0x19999990")
            etree.SubElement(cables, "patchCable", source="velocity", destination="oscAVolume",
                             amount="0x3FFFFFE8")
            etree.SubElement(cables, "patchCable", source="velocity", destination="oscBVolume",
                             amount="0xC0000018")
